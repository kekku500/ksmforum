...
<div class="root_post_wrapper">
    ...
    if($post['parent_post_id'] != null){
      ...
      ?><div class="any_form"><a href="<?php echo site_url($segments_root); ?>"><?php echo 'Algusesse'; ?></a>
          <br><?php
        $segments_upper = array('main', 'topic', $topic['id'], 1, $post['parent_post_id']);
      ?><a href="<?php echo site_url($segments_upper); ?>"><?php echo 'Ülespoole'; ?></a></div><?php
    }
    ?>
      ...
      <?php echo $post['user_name']." - ".$post['post_edit_time']; 
            if ($this->auth->isLoggedIn() && !$post['deleted'] && !$response_disabled){
                //if($this->auth->getUserId() != $post['user_id']){ //kui praegune kasutaja ei loonud seda kommentaari
                    ...
                    <a class = "btn-success" href="<?php echo site_url($segmentsadd); ?>"><?php echo $this->lang->line('post_anchor_add'); ?></a><?php 
                //}
                if($this->auth->getUserId() == $post['user_id']){ //looja saab kommentaari muuta ja kustutada
                    ...
                    <a class = "btn-primary" href="<?php echo site_url($segmentsedit); ?>"><?php echo $this->lang->line('post_anchor_edit'); ?></a>
                <?php
                    $segmentsdel = array('main', 'delpost', $post['post_id'], $cur_url_encoded);?>
                    <a class = "btn-danger" href="<?php echo site_url($segmentsdel); ?>"><?php echo $this->lang->line('post_anchor_del'); ?></a>
              <?php
                }
            }
            ?>
